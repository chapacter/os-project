...
class Map:
    def __init__(self, map_width, map_height):
        ...
        self.spawn_count = {"P": 1, "X": 1, "W": 1, "E": random.randint(10, 20)}

    def objectsSpawn(self, obj, level):
        while self.spawn_count[f"{obj}"] > 0:
            y1 = random.randint(
                self.digger["padding"], self.MAP_HEIGHT - self.digger["padding"]
            )
            x1 = random.randint(
                self.digger["padding"], self.MAP_WIDTH - self.digger["padding"]
            )
            if level[y1][x1] == " ":
                level[y1][x1] = f"{obj}"
    ...
    def move(self):
        ...
        if (
                roll == 4
                and self.digger["y"] < self.MAP_HEIGHT - self.digger["padding"] - 1
        ):
            self.digger["y"] += 1
    ...
    def create(self):
        ...
        while self.digger["wallCountdown"] >= 0:
            ...
            if self.digger["wallCountdown"] < 0:
                ...
                self.objectsSpawn("X", self.level)
