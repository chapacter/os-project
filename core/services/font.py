...
class FontService(Service):
    def __init__(self):
        self._fonts_cache = {}
        self._current_font_key = DEFAULT_FONT_KEY
        self._current_font = None

    def init(self, locale=None, font=None):
        locale_manager.init(locale)
        self.set_font(font or DEFAULT_FONT_KEY)
    ...
    def set_font(self, font_value):
        ...
        if font_str.isdigit():
            idx = int(font_str)
            if 0 <= idx <= 2:
                self._current_font_key = font_str
            else:
                self._current_font_key = DEFAULT_FONT_KEY
        else:
            self._current_font_key = font_str
        ...
        font_info = self._get_font_info(self._current_font_key)
        if font_info:
            path = font_info.get("path")
            if path is None:
                self._current_font = pygame.font.Font(None, 24)
            else:
                try:
                    self._current_font = pygame.font.Font(path, 24)
                except Exception:
                    self._current_font = pygame.font.Font(None, 24)
        else:
            try:
                if os.path.exists(font_str):
                    self._current_font = pygame.font.Font(font_str, 24)
                else:
                    self._current_font = pygame.font.Font(None, 24)
            except Exception:
                self._current_font = pygame.font.Font(None, 24)
    ...
    def _get_font_info(self, font_key):
        if font_key.isdigit():
    ...
    def get_font_key(self):
        return self._current_font_key

    def get_font_name(self):
        info = self._get_font_info(self._current_font_key)
        if info:
            return info["name"]
        return self._current_font_key

    def get_font(self, size):
        if size not in self._fonts_cache:
            font_key = self._current_font_key
            font_info = self._get_font_info(font_key)
            if font_info:
                path = font_info.get("path")
                if path is None:
                    self._fonts_cache[size] = pygame.font.Font(None, size)
                else:
                    self._fonts_cache[size] = pygame.font.Font(path, size)
            else:
                try:
                    if os.path.exists(font_key):
                        self._fonts_cache[size] = pygame.font.Font(font_key, size)
                    else:
                        self._fonts_cache[size] = pygame.font.Font(None, size)
                except Exception:
                    self._fonts_cache[size] = pygame.font.Font(None, size)
    ...
    def render(
            self, text, size, color, shadow=None, outline=None, align="left", antialias=True
    ):
        ...
        if shadow:
            ...
            shadow_surf = self._render_shadow_layer(text, size, shadow_color)
    ...
    def _render_shadow_layer(self, text, size, color):
        font = self.get_font(size)
        return font.render(text, True, color)

    def render_shadow(self, text, size, color, shadow_offset=2, shadow_color=(0, 0, 0)):
        font = self.get_font(size)
    ...
    def render_outline(self, text, size, color, outline_color=(0, 0, 0), border=1):
        font = self.get_font(size)
    ...
    def render_glow(self, text, size, color, glow_color=(255, 215, 0), intensity=2):
        font = self.get_font(size)
        antialias = True
